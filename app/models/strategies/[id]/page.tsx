...
`
    ...
    useEffect(() => {
        (async () => {
            try {
                const res = await fetch(`${API}/strategies/${encodeURIComponent(strategySlug)}/runs`, { cache: "no-store" });
                if (res.ok) {
                                    ...
                                    <div style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(3, 1fr)" }}>
                                        {PARAM_KEYS.map((k: ParamKey) => (
                                            <div key={k} style={{ textAlign: "left" }}>
                                                ...
                                                <input type="number" value={paramsState[k]} onChange={(e) => {
                                                    const next = Number(e.target.value);
                                                    setParamsState((p) => ({ ...p, [k]: next } as ParamsState));
                                                    setJobError(null);
                                                }} style={control} />
                                            </div>
                                        ))}
                                ...
                                <div style={{ ...panel, padding: "1rem" }}>
                                    ...
                                    <div style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(2, 1fr)" }}>
                                        <div style={{ textAlign: "left" }}>
                                            ...
                                            <select value={dataset.exchange} onChange={(e) => {
                                                setDataset((d) => ({ ...d, exchange: e.target.value }));
                                                setJobError(null);
                                            }} style={{ ...control, appearance: "auto" as React.CSSProperties["appearance"] }}>
                                        ...
                                        <div style={{ textAlign: "left" }}>
                                            ...
                                            <select value={dataset.timeframe} onChange={(e) => {
                                        ...
                                        <div style={{ gridColumn: "1 / -1", textAlign: "left" }}>
                                            ...
                                            <input value={dataset.pairs} onChange={(e) => {
                                                ...
                                                setJobError(null);
                                            }} placeholder="e.g. BTC/USDT,ETH/USDT" style={control} />
                                        </div>
                                        <div style={{ textAlign: "left" }}>
