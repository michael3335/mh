...
class AsciiFilter {
  ...
  constructor(
    renderer: THREE.WebGLRenderer,
    { fontSize, fontFamily, charset, invert, interactive = false }: AsciiFilterOptions = {}
  ) {
...
class CanvAscii {
  ...
  constructor(
    { text, asciiFontSize, textFontSize, textColor, planeBaseHeight, enableWaves, interactive = false }: CanvAsciiOptions,
  ...
  private fitToView(padding = 0.92) {
    const cam = this.camera as THREE.PerspectiveCamera;
    const vHalfFovRad = THREE.MathUtils.degToRad(cam.fov / 2);
    const visibleHeight = 2 * cam.position.z * Math.tan(vHalfFovRad);
    const visibleWidth = visibleHeight * cam.aspect;

    const planeParams = this.geometry.parameters as { width?: number; height?: number };
    const planeW = planeParams.width ?? 1;
    const planeH = planeParams.height ?? 1;

    const scale = Math.min(
      (visibleWidth * padding) / planeW,
      (visibleHeight * padding) / planeH
    );
  ...
  setMesh() {
    ...
    this.fitToView(); // ⬅️ ensure initial fit within the container
  }

  setRenderer() {
    this.renderer = new THREE.WebGLRenderer({ antialias: false, alpha: true });
    this.renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, 2)); // DPR aware, capped for perf
  ...
  setSize(w: number, h: number) {
    ...
    this.fitToView(); // ⬅️ refit on resize
  }
  ...
  render() {
    ...
    this.filter.hue(time);
  }

  updateRotation(time?: number) {
    if (!this.interactive) {
      // gentle idle motion, independent of cursor (slightly reduced amplitude to avoid edge kisses)
      const t = time ?? 0;
      const targetX = Math.sin(t * 0.25) * 0.09;
      const targetY = Math.cos(t * 0.20) * 0.09;
    ...
    this.scene.traverse(object => {
      ...
      [obj.material].flat().forEach(material => {
        ...
        Object.keys(materialProps).forEach(key => {
          const matProp = materialProps[key];
          if (
            matProp &&
  ...
  return (
    <div
      ...
      style={{
        ...
        height: '100%',
        touchAction: 'none',
        overflow: 'hidden',                 // ⬅️ clip any spill
        contain: 'paint style layout size', // ⬅️ isolate paints
      }}
    >
      <style>{`
        ...
        .ascii-text-container pre {
          margin: 0;
          user-select: none;
          padding: 0 0.75ch;                /* small L/R gutter to avoid edge clipping */
          line-height: 1em;
          text-align: left;
          position: absolute;
          left: 50%;                         /* center it */
